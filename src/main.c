...
static const Family families[] = {
    {"code_analyzer", "classic-six.code_analyzer", "classic-six", "analysisFocus", "header-contract-audit"},
    {"data_processing", "classic-six.data_processing", "classic-six", "dataWindow", "csv-stream-reconciliation"},
    {"jargon", "classic-six.jargon", "classic-six", "languagePolicy", "c-ecosystem-glossary"},
    {"metrics", "classic-six.metrics", "classic-six", "signalBlend", "latency-error-saturation"},
    {"network_activity", "classic-six.network_activity", "classic-six", "transportMix", "socket-http-sse"},
    {"system_monitoring", "classic-six.system_monitoring", "classic-six", "telemetryScope", "runtime-build-host"},
    {"agent_workflows", "modern-core.agent_workflows", "modern-core", "coordinationMode", "process-orchestration-handshake"},
    {"platform_engineering", "modern-core.platform_engineering", "modern-core", "platformSurface", "make-clang-release-lane"},
    {"observability_ai_runtime", "modern-core.observability_ai_runtime", "modern-core", "runtimeSignals", "logs-metrics-provider-audit"},
    {"delivery_preview_ops", "modern-core.delivery_preview_ops", "modern-core", "deliveryGuardrail", "preview-release-checkpoints"},
    {"supply_chain_security", "modern-core.supply_chain_security", "modern-core", "supplyChainPosture", "binary-integrity-attestation"},
...
static void print_payload(const Family *family, const char *seed, const char *output_format) {
  ...
  if (strcmp(output_format, "json") == 0) {
    ...
    printf("\"message\":\"Deterministic c tranche for %s\",", family->id);
    printf("\"timestamp\":\"2026-01-01T%02u:%02u:%02uZ\",", hour, minute, second);
    printf("\"context\":{\"rendererKey\":\"%s\",\"%s\":\"%s\",\"seedFingerprint\":\"%s-%x\",\"tranche\":\"%s\",\"cProfile\":\"next-20-deterministic-foundation\"},",
           family->renderer, family->context_key, family->context_value, registry, hash, family->tranche);
    printf("\"generationProvenance\":{\"sourceRepo\":\"c-stakeholder\",\"baseline\":\"next20-family-focus\",\"experimental\":false,\"adapterType\":\"static-catalog\",\"promptVersion\":null},");
  ...
  printf("message: Deterministic c tranche for %s\n", family->id);
}
